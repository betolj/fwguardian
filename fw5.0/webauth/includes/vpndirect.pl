...
# "POST /admin/chvpndirect.cgi" -> save or reload button
sub chvpndirect {
    my $s = shift;
    ...
    if ($rlfw == 0 && $canch == 1) {
       ...
       foreach my $auxjson (split /"[0-9]+":/, $s) {
          ...
          if ($auxjson && $auxjson ne "") {
             ...
             if ($json{'Group'} =~ /^DIRECT/ && $json{'ifTun'} ne "" && $json{'Src'} ne "" && $json{'Dst'} ne "") {
                # vpn.conf rules
                $canSync = 1;
                if ($json{'DstLAN'} eq "" || $json{'DstLAN'} eq "none") {
                   $json{'Dgd'} = "none";
                   $json{'DstLAN'} = "any";
                }
                $auxentry = "$json{'ifTun'} $json{'Src'} $json{'Dst'} $json{'fwTarg'}\t$json{'DstLAN'}";

                if ($json{'fwTarg'} eq "TUNNEL") {
                   $auxentry = "$auxentry $json{'Dgd'}";
                   $auxentry = "$auxentry dgd-reload" if ($json{'Dgd'} ne "none" && $json{'dreload'} eq "Yes");
                   $auxentry = "$auxentry with-ipip" if ($json{'ipip'} eq "Yes");
                   if ($json{'ipsecp'} ne "none" && $json{'ipsecsp'} =~ /[0-9]+\/[0-9]+/) {
                      $auxentry = "$auxentry ipsec-psk=\"$json{'ipsecp'}\"";
                      $auxentry = "$auxentry spi=$json{'ipsecsp'}";
                   }
                }
                else {
                   $auxentry = "$auxentry none";
                }
       ...
       my $meta = "<head><META HTTP-EQUIV=\"Refresh\" CONTENT=\"$rtime;URL=/admin/vpndirect.cgi\"><META http-equiv=\"content-type\" content=\"text/html;charset=utf-8\"></head>";
...
# "Make web page vpndirect.html"
sub get_vpndirect {

    my $htmlfile="$HTMLDIR/admin/dynhttp/vpndirect.html";
    read_fwcfg;
    read_profiles;
  ...
  <script type="text/javascript">
     jQuery(document).ready(function(){
        ...
        jQuery("#fwVpnGrid").jqGrid({
           url:'/admin/getvpndirect.json',
           datatype: "json",
           height: \$(window).height() - 300,
           width: \$(window).width() - 80,
javascript
if ($FW_LANG == 0) {
    print FILE "           colNames:['ID', 'Politica', 'Interface', 'Endereço', 'End. Remoto(TUNNEL)<BR />End. Origem(ROUTE)', 'Alvo', 'LAN remota', 'End. Teste', 'Reinicia', 'Roteamento<BR />IPIP', 'Senha PSK', 'SPI', 'Condição', 'Descrição', 'Control' ],\n";
}
else {
    print FILE "           colNames:['ID', 'Policy', 'Interface', 'Address', 'Remote Addr(TUNNEL)<BR />Source Addr(ROUTE)', 'Target', 'Remote LAN', 'Test Addr', 'Reload', 'IPIP<BR />Routing', 'PSK password', 'SPI', 'Condition', 'Description', 'Control' ],\n";
}
print FILE << "javascript";
           colModel: [
...
print FILE << "javascript";
              { name:"ifTun",   index:'ifTun',   sortable:false, editable:true, width:78 },
              { name:"Src",     index:'Src',     sortable:false, editable:true, width:140 },
              { name:"Dst",     index:'Dst',     sortable:false, editable:true, width:220 },
              { name:"fwTarg",  index:'fwTarg',  sortable:false, editable:true, edittype:'select', editoptions:{value:"TUNNEL:TUNNEL;ROUTE:ROUTE"}, width:78 },
              { name:"DstLAN",  index:'DstLAN',  sortable:false, editable:true, width:120 },
              { name:"Dgd",     index:'Dgd',     sortable:false, editable:true, hidden:true, width:80 },
              { name:"dreload", index:'dreload', sortable:false, editable:true, edittype:"checkbox", editoptions:{value:"Yes:No"}, hidden:true, width:60 },
              { name:"ipip",    index:'ipip',    sortable:false, editable:true, edittype:"checkbox", editoptions:{value:"Yes:No"}, hidden:true, width:90 },
              { name:"ipsecp",  index:'ipsecp',  sortable:false, editable:true, dataType:'string', width:180 },
              { name:"ipsecsp", index:'ipsecsp', sortable:false, editable:true, dataType:'string', defval:'100/101', width:80 },
javascript
...
print FILE << "javascript";
           ...
           ondblClickRow: function (selid, iRow,iCol) {
             if (document.getElementById('mvPol').checked == true) {
                 ...
                 editRow(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "$valias", "vpndirect");
                 newRow = updnewRow();
              }
           },
           groupingView : {
           ...
           gridComplete: function(data, response) {
              rulesGrid=GridComplete(jQuery("#fwVpnGrid"), rulesGrid, rulesCt, saveall, "vpndirect");

              rulesCt++;
              jQuery("#fwVpnGrid").closest(".ui-jqgrid-bdiv").scrollTop(scrollPosition);
           },
javascript
$msg[0] = "Tunelamento direto (chaves manuais)";
$msg[1] = "Direct Tunneling (manual keys)";
print FILE "           caption: '$msg[$FW_LANG]'\n";
$msg[0] = "Configuração IPSec";
$msg[1] = "IPSec configuration";
print FILE << "javascript";
        });
        jQuery("#fwVpnGrid").jqGrid('setGroupHeaders', {
             useColSpanStyle: true, 
             groupHeaders:[
               {startColumnName: 'ifTun', numberOfColumns: 2, titleText: '<font size="2">TUN</font>'},
               {startColumnName: 'Dgd',   numberOfColumns: 2, titleText: '<font size="2">DGD - failover</font>'},
               {startColumnName: 'ipsecp', numberOfColumns: 2, titleText: '<font size="2">$msg[$FW_LANG]</font>'} ]
        });
        ...
        jQuery("#gdUp").click( function() {
           rulesGrid = mvUp(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "vpndirect");
           newRow = updnewRow();
        ...
        jQuery("#gdDown").click( function() {
           rulesGrid = mvDown(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "vpndirect");
           newRow = updnewRow();
           doReload = upddoReload();
        });
...
print FILE << "javascript";
           onClickButton:function(){
               \$("#fwVpnGrid").showCol("Dgd");
               \$("#fwVpnGrid").showCol("dreload");
               \$("#fwVpnGrid").showCol("ipip");
           }
        });
...
print FILE << "javascript";
           onClickButton:function(){
             ...
             if (gridrules > 0) {
                ...
                rulesGrid = cloneRow(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "vpndirect", clret['Group']);
                newRow = updnewRow();
             }
           }
        });
...
print FILE << "javascript";
           onClickButton:function(){
             ...
             rulesGrid = addRow(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "vpndirect", defGroup, "$valias");
             newRow = updnewRow();
           }
        });
...
print FILE << "javascript";
           ...
           saveall = 1;
           saveAll(jQuery("#fwVpnGrid"), rulesGrid, newRow, "$medited[$FW_LANG]", "$msg[$FW_LANG]", "vpndirect", "/admin/getvpndirect.json", "/admin/chvpndirect.cgi");
           newRow = updnewRow();
...
javascript
    ...
    $msg[0] = "VPN: Tunelamento direto!";
    $msg[1] = "VPN: Direct Tunneling!";
...
HTMLCODE
   ...
   ## Grid rules
   print FILE "<FORM name=\"fivpn\" action=\"/admin/chvpndirect.cgi\" method=\"post\">";
